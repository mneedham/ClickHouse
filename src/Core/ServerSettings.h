...
#define SERVER_SETTINGS(M, ALIAS) \
    ...
    M(Int64, heap_profiler_log_sample_rate, 25, "asdqwe, 0 to sample every allocation (very slow), -1 to disable", 0) \
    M(Int64, heap_profiler_max_samples, -1, "asdqwe, each takes asdqwe bytes, -1 to pick automatically (constant * available memory / 2^heap_profiler_log_sample_rate). With default settings, heap profiling uses tens of MB of memory.", 0) \
    M(Int64, heap_profiler_dump_period_seconds, 180, "asdqwe", 0) \
